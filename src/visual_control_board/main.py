...
app.mount("/static", StaticFiles(directory=package_base_dir / "static"), name="static")
...
async def startup_event():
    ...
    app.state.current_actions_config: Optional[ActionsConfig] = (
        config_loader_instance.actions_config
    )
    ...
    if app.state.current_ui_config is None:
        logger.critical(
            "CRITICAL: Failed to load initial UI configuration. Application cannot start."
        )
        raise RuntimeError(
            "Failed to load initial UI configuration. Check logs for details."
        )
    else:
        logger.info(
            f"Initial UI configuration loaded: {len(app.state.current_ui_config.pages)} page(s)."
        )

    # Critical check: Initial Actions Configuration must be loaded.
    if app.state.current_actions_config is None:
        logger.critical(
            "CRITICAL: Failed to load initial Actions configuration. Application cannot start."
        )
        raise RuntimeError(
            "Failed to load initial Actions configuration. Check logs for details."
        )
    else:
        logger.info(
            f"Initial Actions configuration loaded: {len(app.state.current_actions_config.actions)} action(s)."
        )

    # Initialize Action Registry with current actions config
    action_registry_instance = ActionRegistry()
    action_registry_instance.load_actions(
        actions_config=app.state.current_actions_config
    )
